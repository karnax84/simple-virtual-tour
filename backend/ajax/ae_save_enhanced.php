...
if(!$autoenhance_create) {
    ob_end_clean();
    echo json_encode(array("status"=>"error","msg"=>"insufficient credits"));
    exit;
}
$image_url = autoenhance_enhanced_image($image_id);
...
if(file_exists($path_panorama_original_new) && $fileSizeKB > 100) {
    ...
    $mysqli->query("UPDATE svt_autoenhance_log SET processed=1 WHERE id_room=$id_room AND id_image='$image_id';");
