...
if(isset($_FILES) && !empty($_FILES['file']['name'])){
    ...
    if(in_array($ext,$allowed_ext)){
        ...
        if($moved) {
            ...
            $result = $mysqli->query("UPDATE svt_virtualtours SET dollhouse_glb='$name' WHERE id=".$_SESSION['id_virtualtour_sel']);
            if($result) {
                ob_end_clean();
                echo $name;
            } else {
        ...
        ob_end_clean();
        echo 'ERROR: '._("Only gltf,glb files are supported.");
    }
} else {
    ob_end_clean();
