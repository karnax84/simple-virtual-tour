...
if(isset($_FILES) && !empty($_FILES['file']['name'])){
    ...
    if(in_array($ext,$allowed_ext)){
        $video = "pano_".round(microtime(true) * 1000).".".$ext;
        if($s3_enabled) {
            $moved = move_uploaded_file($_FILES['file']['tmp_name'],"s3://$s3_bucket_name/viewer/videos/".$video);
        } else {
        ...
        if($moved) {
            ...
            ob_end_clean();
            if($s3_enabled) {
                echo $s3_url.'viewer/videos/'.$video;
            } else {
        ...
        ob_end_clean();
        echo 'ERROR: '._("Only mp4, webm files are supported.");
    }
}else{
    ob_end_clean();
