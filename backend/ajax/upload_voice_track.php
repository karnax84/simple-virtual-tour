...
session_write_close();
if(isset($_FILES) && !empty($_FILES['audio_data']['name'])){
    ...
    $moved = move_uploaded_file($_FILES['audio_data']['tmp_name'],$path_dest);
    if($moved) {
        if($s3_enabled && $settings['aws_s3_type']=='digitalocean') {
            try {
                $s3Client->putObjectAcl([
                    'Bucket' => $s3_bucket_name,
                    'Key' => "video/assets/$name",
                    'ACL' => 'public-read',
                ]);
        ...
        ob_end_clean();
        echo 'ERROR: code:'.$_FILES["audio_data"]["error"];
    }
}else{
    ob_end_clean();
