...
if($user_info['role']!='administrator') {
    if ($user_info['id_plan'] != 0) {
        ...
        if (!empty($customize_menu_json)) {
            ...
            if(isset($customize_menu['custom5'])) {
                if ($customize_menu['custom5'] == 0) {
                    $custom_menu = false;
                }
            }
        }
...
if(!empty($extra_menu_items)) {
    ...
    if(empty($extra_menu_items[4]['link']) || $extra_menu_items[4]['type']!='iframe') {
        $custom_menu = false;
    }
} else {
    $custom_menu = false;
...
<div style="display: none;margin-bottom:-10px;" id="iframe_div">
   <iframe allow='accelerometer; camera; display-capture; fullscreen; geolocation; gyroscope; magnetometer; microphone; midi; xr-spatial-tracking;' width="100%" height="100vh" frameborder="0" scrolling="yes" marginheight="0" marginwidth="0" src="<?php echo $extra_menu_items[4]['link']; ?>"></iframe>
</div>
