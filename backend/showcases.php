...
$can_create = get_plan_permission($id_user)['create_showcase'];
...
<?php if(!$can_create) : ?>
    <div class="card bg-warning text-white shadow mb-4">
        <div class="card-body">
            <?php echo sprintf(_('Your "%s" plan not allow to create Showcases!'),$user_info['plan'])." ".$msg_change_plan; ?>
        </div>
    </div>
<?php exit; endif; ?>
...
<div class="row">
        ...
        <div class="card mb-4 py-3 border-left-success">
            <div class="card-body" style="padding-top: 0;padding-bottom: 0;">
                <form autocomplete="off">
                    <div id="modal_new_showcase" class="row align-items-end">
                        ...
                        <div class="col-md-12 col-lg-6 mt-3 mt-lg-0 text-lg-right text-center">
                            <div class="form-group mb-0">
                                <button <?php echo ($demo) ? 'disabled':''; ?> onclick="add_showcase();" type="button"  class="btn btn-success"><i class="fas fa-plus"></i> <?php echo _("Create"); ?></button>
                            </div>
                        </div>
                    </div>
                </form>
    ...
    <div class="col-md-12">
        <div class="card shadow mb-4">
            <div class="card-body">
                <table class="table table-bordered table-hover" id="showcases_table" width="100%" cellspacing="0">
                    <thead>
                    <tr>
...
<script>
    (function($) {
        ...
        $(document).ready(function () {
            $('#showcases_table').DataTable({
                "responsive": true,
                "scrollX": true,
                "processing": true,
                "searching": true,
                "serverSide": true,
                "ajax": "ajax/get_showcases.php",
                "drawCallback": function( settings ) {
                    $('#showcases_table').DataTable().columns.adjust();
                },
                "language": {
                    "decimal":        "",
            ...
            $('#showcases_table tbody').on('click', 'td', function () {
                var showcase_id = $(this).parent().attr("id");
                location.href = 'index.php?p=edit_showcase&id='+showcase_id;
            });
        });
    })(jQuery);
</script>