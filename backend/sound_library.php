...
$can_create = get_plan_permission($id_user)['enable_sound_library'];
...
if($virtual_tour!==false) {
    ...
    if($user_info['role']=='editor') {
        ...
        if($editor_permissions['sound_library']==0) {
            $sound_library = false;
        }
    }
...
<?php if($virtual_tour['external']==1) : ?>
    <div class="card bg-warning text-white shadow mb-4">
        <div class="card-body">
            <?php echo _("You cannot use Sound Library on an external virtual tour!"); ?>
        </div>
    </div>
<?php exit; endif; ?>
...
<?php if(!$can_create) : ?>
    <div class="card bg-warning text-white shadow mb-4">
        <div class="card-body">
            <?php echo sprintf(_('Your "%s" plan not allow to create Sound Library!'),$user_info['plan'])." ".$msg_change_plan; ?>
        </div>
    </div>
<?php exit; endif; ?>
    ...
    (function($) {
        ...
        $(document).ready(function () {
            ...
            var music_files_dropzone = new Dropzone("#music-dropzone", {
                ...
                acceptedFiles: 'audio/mpeg,audio/wav,audio/wave,audio/x-wav'
            });
            ...
            music_files_dropzone.on("success", function(file,rsp) {
                add_file_to_sound_library(id_virtualtour,rsp);
            });
